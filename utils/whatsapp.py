...
def generar_msg_factura(row) -> str:
    ...
    return (
        f"*Zytro Vision - Detalle de su Servicio*\n\n"
        ...
        f"Estado: {estado}\n\n"
        f"Para consultas: +593 96 324 1158 | Zytro Vision"
    )
...
def generar_msg_hc(row, paciente_info) -> str:
    ...
    return (
        f"*Zytro Vision - Resumen de Consulta Optometrica*\n\n"
...
def generar_msg_indicaciones(row, paciente_info) -> str:
    ...
    return (
        f"*Zytro Vision - Indicaciones Medicas*\n\n"
        f"Paciente: {paciente_info.get('nombre', '')}\n"
        f"Consulta: {row.get('fecha', '')}\n\n"
        f"*Indicaciones / Tratamiento:*\n{recom}"
        f"{control_txt}\n\n"
        f"Consultas: +593 96 324 1158 | Zytro Vision"
    )
